...
%% 数值积分公式采用复化Simpson公式和Gauss型求积公式
...
for ni=1:1:MK
    ...
    G=zeros(5, N);
    for i=1:1:N
        G(1, i)=2*i-1;  G(2, i)=2*i+1;  G(3, i)=2*i; 
        G(4, i)=p(i);   G(5, i)=p(i+1)-p(i);
    end
...
%     xi = GaussianQPoints(0, 1, NM);
    ...
    for i=1:1:N
        ...
        temp1=u(G(1, i))*VecN(1, :)+u(G(2, i))*VecN(2, :)+u(G(3, i))*VecN(3, :)-VecU;
        temp2=u(G(1, i))*VecM(1, :)/h+u(G(2, i))*VecM(2, :)/h+u(G(3, i))*VecM(3, :)/h-VecDu;
