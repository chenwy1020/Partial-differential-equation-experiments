...
for ni=1:1:MK  
    ...
    for i=1:1:Ne
        ...
        if(e(5,i)==2)
            [intik ,intkj]=triangleTBoundary2(x1,y1,x2,y2,D,dxu,dyu);
            b(e(1,i))=b(e(1,i))-intik;
            b(e(2,i))=b(e(2,i))-intkj;
        end

        if(e(5,i)==3)
            [intik ,intkj]=triangleTBoundary3(x1,y1,x2,y2,D,dxu,dyu);
            b(e(1,i))=b(e(1,i))-intik;
            b(e(2,i))=b(e(2,i))-intkj;
        end
...
hold on
loglog(h, h.^2/30,   '-.g', 'DisplayName', 'O(h^2)');

loglog(h, err1, '--square k', 'DisplayName', '||u-u_{h}||_{H^{1}}');
loglog(h, h/4,    '-.b', 'DisplayName', 'O(h)')

loglog(h, errC, '--diamond k', 'DisplayName', '||u-u_{h}||_{L^{\infty}}');
loglog(h, h.^1.8/8, '-.r', 'DisplayName', 'O(h^{1.8})');
...
function [intik ,intkj]=triangleTBoundary2(x1,y1,x2,y2,D,dxu,dyu)
    w=[5 8 5]'/9.0;
    a=0; b=1;
    t1=(a+b)/2-(b-a)/2*sqrt(3/5.0);
    t2=(a+b)/2;
    ...
    f=@(t)([-(yk1-y1), xk1-x1]*D(x1+(xk1-x1)*t,y1+(yk1-y1)*t)*[dxu(1,y1+(yk1-y1)*t), dyu(1,y1+(yk1-y1)*t)]');
    intik=[f(t1),f(t2),f(t3)]*w/2;

    f=@(t)([-(yk1-y2), xk1-x2]*D(x2+(xk1-x2)*t,y2+(yk1-y1)*t)*[dxu(1,y2+(yk1-y2)*t), dyu(1,y2+(yk1-y2)*t)]');
    intkj=-[f(t1),f(t2),f(t3)]*w/2;

end

function [intik ,intkj]=triangleTBoundary3(x1,y1,x2,y2,D,dxu,dyu)
    w=[5 8 5]'/9.0;
    a=0; b=1;
    t1=(a+b)/2-(b-a)/2*sqrt(3/5.0);
    t2=(a+b)/2;
    ...
    f=@(t)([-(yk1-y1), xk1-x1]*D(x1+(xk1-x1)*t,y1+(yk1-y1)*t)*[dxu(x1+(xk1-x1)*t,1), dyu(x1+(xk1-x1)*t,1)]');
    intik=[f(t1),f(t2),f(t3)]*w/2;

    f=@(t)([-(yk1-y2), xk1-x2]*D(x2+(xk1-x2)*t,y2+(yk1-y1)*t)*[dxu(x2+(xk1-x2)*t,1), dyu(x2+(xk1-x2)*t,1)]');
    intkj=-[f(t1),f(t2),f(t3)]*w/2;
