...
for ni=1:1:MK
    n=10*ni;
    A=zeros(2*n+1, 2*n+1);   y=zeros(2*n+1, 1);     u=zeros(2*n+1, 1);
    points=zeros(2*n+1, 1);  dualpoints=zeros(2*n+2, 1);
    
    %% 网格信息
    % 均匀剖分,均匀对偶剖分
    h=(xmax-xmin)/n;
    points(:,1)=xmin+(0:1:2*n)*h/2;   

    a=0; b=h;
    t=(a+b)/2-(b-a)/2/sqrt(3);
    dualpoints(2:2:2*n,1)=points(1:2:2*n-1)+t;
    t=(a+b)/2+(b-a)/2/sqrt(3);
    dualpoints(3:2:2*n+1,1)=points(1:2:2*n-1)+t;
    ...
    for i=1:1:n
        ...
        err2(ni)=err2(ni)+(5*temp(1)+8*temp(2)+5*temp(3))/9.0;

        % 计算 H1 半范数
        temp=(tempdu-tempdv).^2*hi/2.0;
        errf(ni)=errf(ni)+(5*temp(1)+8*temp(2)+5*temp(3))/9.0;
    end
