...
for ni=1:1:MK  
    ...
    %% 网格信息
    mesh1=generateMesh(model,'GeometricOrder','linear','Hmax',h0(ni));
    ...
    for i=1:1:length(t)
        ...
        % u 和 真解的导数值
        tempdux=[DVecy; DVecy; DVecy; DVecy]*[u(t(1 ,i)) u(t(2 ,i)) u(t(3 ,i))]'/(2*S);
        tempduy=[DVecx; DVecx; DVecx; DVecx]*[u(t(1 ,i)) u(t(2 ,i)) u(t(3 ,i))]'/(2*S);
...
figure
scatter3(p(1,:),p(2,:),u,'black','filled');
hold on
scatter3(p(1,:),p(2,:),V,'red','filled');
hold off
...
hold on
loglog(h, h*5, '-.k', 'DisplayName', 'O(h)');
legend('Show');
hold off
